...
double qbeta(double alpha, double p, double q, int lower_tail, int log_p)
{
    ...
    qbeta_raw(alpha, p, q, Rboolean(lower_tail != 0), Rboolean(log_p != 0),
	      // log_q_cut ,      n_N
...
maybe_swap:
    ...
    Rboolean u0_maybe = Rboolean(M_LN2 * DBL_MIN_EXP < u0 && u0 < -0.01);
